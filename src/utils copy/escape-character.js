...
// 全部替换方法
String.prototype.codeReplaceAll = function(s1, s2) {
...
export function tw2sw_string(txt) {
  transWordList.forEach(function(value, index, array) {
    txt = txt.codeReplaceAll(value, specialwordList[index])
  })
  return txt
}
...
export function sw2tw_string(txt) {
  const index = transWordList.findIndex(i => i === txt)
  return specialwordList[index]
}

// 过滤具有特殊字符的字符串
export function filterswString(str) {
  const reg = /&[a-z]+;/g
  return str.replace(reg, sw2tw_string)
}
